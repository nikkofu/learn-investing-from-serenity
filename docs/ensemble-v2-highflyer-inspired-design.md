...
### 5.1 成员收益相关矩阵（T1 · pooled 跨标的 · in-sample）

|  | cw-v4 | cw-v3 | chk-v5 | chan-rev | rsi-rev |
|---|---|---|---|---|---|
| cw-v4 | 1.00 | **0.92** | **0.78** | 0.25 | 0.06 |
| cw-v3 | | 1.00 | **0.75** | 0.21 | 0.06 |
| chk-v5 | | | 1.00 | 0.26 | 0.05 |
| chan-rev | | | | 1.00 | 0.00 |
| rsi-rev | | | | | 1.00 |

**坐实参考文档"防虚假共振"**：3 个趋势核心高度同源（0.75~0.92），加权只是放大同一 beta；两个回归成员与趋势近正交（rsi-rev 几乎与所有人 0 相关）。

### 5.2 配权方案对比（T1 · 12 只 · in-sample）

| 方案 | ret% | maxDD% | sharpe | posStocks% | 有效成员数 | ret/\|DD\| |
|---|---|---|---|---|---|---|
| fixed(v1) | 57.7 | −16.6 | 0.13 | 50 | 4.72 | 3.48 |
| **equal** | **58.0** | **−16.0** | **0.16** | 50 | **5.00** | **3.62** |
| invVol | 25.3 | −10.5 | 0.21 | 58 | 2.02 | 2.41 |
| riskParity(ERC) | −0.2 | −1.9 | — | 33 | 1.10 | 退化 |
| corrPenalized | 48.9 | −15.2 | 0.15 | 50 | 4.46 | 3.21 |

**结论**：等权全面小胜手调；朴素 `invVol`/ERC 退化（权重被极低波动的 rsi-rev 吸走），须加上下限夹逼（`invVolCapped`）。故 V2 基底选 **equal**。

### 5.3 V2 消融与风控闸门调参（T4 · 12 只）

| 变体 | ret% | maxDD% | sharpe | ret/\|DD\| |
|---|---|---|---|---|
| v1 | 57.7 | −16.6 | 0.13 | 3.48 |
| **v2（equal + gate s−.12/h−.18）** | **52.3** | **−13.3** | **0.22** | **3.94** |
| v2-noGate（equal only） | 58.0 | −16.0 | 0.16 | 3.62 |
| v2 + trendClusterCap 0.5 | 47.7 | −12.7 | 0.22 | 3.74 |
| v2-invVolCapped | 48.4 | −15.0 | 0.19 | 3.22 |
| v2 + volGate 0.03/20 | 25.4 | −12.2 | 0.20 | 2.08 |

**发现**：风控闸门是回撤改善主力（−16.0→−13.3）；`trendClusterCap` 在等权基底下以 ~4.6pt 收益换 ~0.6pt 回撤，得不偿失 → **默认关闭**；波动闸门在本篮子严重伤收益 → **默认关闭**。二者均保留为可选旋钮。风控阈值以 `ddSoft −12% / ddHard −18% / cooldown 0` 为最优（夏普 0.22、单位回撤收益 3.94）。

### 5.4 最终 `ENSEMBLE_V2_DEFAULTS`

`weightScheme: "equal"`、`trendClusterCap: 0`、`riskGate: true`、`ddSoft: −0.12`、`ddHard: −0.18`、`cooldownBars: 0`、`volLen: 0`、`volCap: 0`；其余（成员清单、posCap 0.95、regime 参数）沿用 V1。

### 5.5 §3 验收对照

| 验收项 | 目标 | ensemble-v2 | |
|---|---|---|---|
| 最大回撤 | 严格优于 v1(−16.6%) | **−13.3%** | ✅ |
| 平均收益 | ≥ v1 的 90%(≈51.9%) | **+52.3%** | ✅ |
| 分散度 | 有效成员数较 v1 提升 | **5.00**(v1 4.72) | ✅ |
| 盈利股占比 | ≥ 50% | **50%** | ✅ |
| 无未来函数 | 配权走前 + 风控因果 | 走前滚动 + 因果净值 | ✅ |
| 夏普 / 单位回撤收益 | ≥ v1 | **0.22 / 3.94**(v1 0.13 / 3.48) | ✅ |

### 5.6 诚实取舍

- V2 以约 V1 的 83% 收益换取更低回撤、更高风险调整收益，符合本轮"冲稳健性/一致性"的定位；**仍不跑赢买入持有**（+150%，单边强牛里任何择时/降仓都牺牲尾部）。
- 参数经 12 只篮子择优，有过拟合风险；风控阈值为篮子级最优，换标的/周期需重扫。
- 远期（未做）：正交的 LLM 语义/事件因子（复用现有 AI 研判层，验证与量价相关性≈0）、日内分时段路由（需分时数据管线）、GP/RL 类训练能力。
