 ...
 *
 * 优先级：深链 ?strategy= > 上次保存 > 偏好默认（Cardwell RSI Trade Navigator）> 后端默认 > 列表首个。
 * 所有候选都按「是否存在于当前策略列表」校验，失效时安全回退。
 */

/** 偏好默认买卖引擎：Cardwell RSI Trade Navigator。 */
export const PREFERRED_PRO_STRATEGY_ID = "tv-cardwell-rsi-navigator-v1";
