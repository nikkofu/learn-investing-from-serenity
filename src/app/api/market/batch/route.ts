...
// 带指数退避的重试请求：对 502/503/429 等瞬态网关错误更宽容
async function fetchWithRetry(url: string, options: RequestInit, retries = 5, baseDelay = 800): Promise<Response> {
  let lastError: any;
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // 对 502/503/429 等瞬态错误增加等待后重试；4xx/5xx 其他错误也重试但间隔更短
      lastError = new Error(`HTTP 错误: ${res.status}`);
    } catch (err) {
      lastError = err;
    }
    if (i < retries - 1) {
      // 指数退避：800ms -> 1600ms -> 3200ms -> 6400ms
      const jitter = Math.random() * 200;
      await new Promise((resolve) => setTimeout(resolve, baseDelay * Math.pow(2, i) + jitter));
    }
  }
  throw lastError || new Error("请求失败且已超过最大重试次数");
}

// 分批拆分工具：将大数组拆为固定大小的子数组
  ...
  try {
    ...
    const list = await globalCache.getOrCreate(
      ...
      async () => {
        ...
        // 分批拉取：每批最多 15 只股票，避免 URL 过长触发上游网关限流/502
        const BATCH_SIZE = 15;
        const codeChunks = chunkArray(codeArr, BATCH_SIZE);
        const allDiffItems: any[] = [];
        ...
        for (const chunk of codeChunks) {
          ...
          allDiffItems.push(...diffList);
        }

        // 将行情数据按代码建档，确保返回结果的顺序一致
        const quoteMap = new Map<string, any>();
        allDiffItems.forEach((q: any) => {
          if (q && q.f12) {
