...
### 新增
- `src/lib/tvStrategies.ts`：新增 `CardwellV4Params` / `buildCardwellV4Ctx` / `computeCardwellRsiNavigatorV4` / `runTvCardwellRsiNavigatorV4(Params)`。默认三条改造：①智能冷却（默认 3 根，收盘站上离场那根高点且 RSI 收复 50 即强反包豁免）；②反追高距离闸门（收盘离 MA20 超 +8% 不进）；③稳定 R 单位 TP/SL（R=近 20 根 ATR 中位数，SL 取入场−1.8R 与近 6 根 swing low 更紧者，TP1/2/3=+1/1.5/2.5R）。入场触发默认沿用 V3「区间切换上冲 + 正向反转」，并内置可选触发「动能启动 / 回踩起涨」（默认关，实测增噪降胜率）。离场沿用 V3 三道（吊灯 3×ATR / RSI 跌破 38 / 连 2 根破 MA30）。
- `src/lib/strategies.ts`：注册 V4 策略卡（`compute` + `backtest`），`/chart` 策略图层、`/analyze`、`/backtest/strategy` 均可选。

### 回测（12 只 A 股 · 360 根日线 · 含双边手续费）
- 平均收益 V4 **+93.2%** vs V3 +81.1%（买入持有 +126.6%）；平均最大回撤 V4 **−22.7%** vs V3 −20.7%；胜率 V4 36% vs V3 39%；年均交易 V4 7.5 vs V3 6.1 笔。
- **诚实口径**：V4 更高的均值几乎全由单只强趋势票 601869（+973% vs +789%）贡献；12 只里 V4 实际在 8 只上略逊于 V3，且回撤更深、胜率更低。300024（用户关注票）V4 −32.5% vs V3 −12.7%——V4 确实抢到了更早的 6/15 @16.96 入场，但该波 6/15 实为假突破、6/26 被结构止损 −10.3%，6/29 才真爆发。故**不切默认**，V4 仅作可选激进变体。
- 每档 TP 历史命中率：TP1 40% / TP2 33% / TP3 29%（共 90 笔入场）。
...
### 修复
- `src/app/settings/page.tsx`：在 `emailConfig` 状态中添加 `maskedSenderEmail` 和 `maskedRecipientEmail` 字段；修复 `fetchEmailConfig` 函数，从 API 返回的脱敏邮箱数据中填充到状态中；修改邮箱输入框的 onChange 事件，当用户修改邮箱时清空脱敏显示。
...
### 新增
- `src/app/api/settings/evening-scan/route.ts`：新增 API 端点，支持手动触发晚间扫描并发送邮件。
- `src/app/settings/page.tsx`：在邮件配置区域新增"立即执行扫描并发送邮件"按钮，添加 `runEveningScanNow` 函数处理点击事件。
...
### 修复
- `src/lib/eveningScan.ts`：在过滤条件中新增第 5 条检查，排除 `channelStatus === "breakdown"` 的股票，确保筛选出的股票在通道内且未破位。
